...
```go
// 定义错误类型用于 HTTP handler 映射
var ErrBadRequest = errors.New("bad request")

eventDispatcher.OnP2MessageReceiveV1(func(ctx context.Context, event *larkim.P2MessageReceiveV1) error {
    // 1. 获取 event_id 用于去重（**注意：需要 nil 检查，返回 ErrBadRequest 映射为 400**）
    if event.Header == nil {
        log.Printf("[Webhook] Event header is nil")
        return ErrBadRequest  // HTTP handler 映射为 400 Bad Request
    }
    eventID := event.Header.EventID  // string 类型，不需要解引用
...
mux.HandleFunc(wr.config.Path, func(w http.ResponseWriter, r *http.Request) {
    ...
    // 4.3 构造 EventReq（**重要：用已读取的 body，不要再读 r.Body**）
    ...
    // 4.4 Challenge 处理（ParseReq 后判断 type）
    // 使用 SDK 的 ParseReq 解析请求，如果是 challenge 直接返回

    // 4.5 签名验证（失败返回 401）
    // 使用 SDK 的 VerifySign，失败返回 http.StatusUnauthorized
    if err := eventDispatcher.VerifySign(eventReq); err != nil {
    ...
    resp := eventDispatcher.DoHandle(ctx, eventReq)
    ...
    if resp.Err != nil {
        ...
        if errors.Is(resp.Err, ErrBadRequest) {
...
**实现建议**：参考 SDK 的 `event/dispatcher` 包的 `Do` 方法，手动调用 `ParseReq`、`VerifySign`、`DecryptEvent`、`DoHandle`，在每个步骤根据错误类型返回正确的 HTTP 状态码。注意用已读取的 `body` 构造 `EventReq`，不要再读 `r.Body`。
...
3. （可选）手动验证：启动 server 后用 curl 测试
   - Challenge: `curl -X POST localhost:8080/webhook/feishu -d '{"type":"url_verification","token":"xxx","challenge":"test_challenge"}' -H 'Content-Type: application/json'` 应返回 `{"challenge":"test_challenge"}`
   - Method: `curl localhost:8080/webhook/feishu` 应返回 405
   - 注意：如果启用了 EncryptKey，challenge 请求也需要加密，测试时可以临时使用空 EncryptKey
  </verify>
  <done>
Start 方法完整实现，包含：
